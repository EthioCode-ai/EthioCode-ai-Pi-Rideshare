...
router.post('/process-video', async (req, res) => {
  try {
    ...
    res.status(202).json({
      success: true,
      data: { contributionId, status: 'processing', message: 'Video upload received. Processing will take a few minutes.' }
    });
...
async function processVideoAsync(contributionId, storeId, videoUrl, userId) {
  ...
  try {
    ...
    for (let i = 0; i < frames.length; i++) {
      ...
      if (detection) {
        detections.push({ ...detection, sequence: i + 1, timestamp: i * 5 });
      }
    }
...
async function extractVideoFrames(videoUrl) {
  ...
  for (let i = 0; i < estimatedDuration; i += interval) {
...
function mockAnalyzeFrame() {
  ...
  if (random < 0.3) {
    ...
    const categories = [['Produce'],['Dairy','Milk'],['Meat','Seafood'],['Frozen'],['Snacks','Chips'],['Beverages'],['Cereal'],['Canned Goods'],['Baby','Pet'],['Health'],['Cleaning'],['Paper Products']];
    return { type: 'aisle', number: String(aisleNum), description: categories[aisleNum-1]?.join(', '), categories: categories[aisleNum-1] || ['General'] };
  } else if (random < 0.4) {
    const areas = ['pharmacy', 'restroom', 'checkout', 'deli', 'bakery', 'entrance'];
    const areaType = areas[Math.floor(Math.random() * areas.length)];
    return { type: 'area', areaType, name: areaType.charAt(0).toUpperCase() + areaType.slice(1) };
  }
  return null;
}
