...
import { useNavigation as useGoogleNav } from '@googlemaps/react-native-navigation-sdk';
...
import type { Location as NavLocation } from '@googlemaps/react-native-navigation-sdk/lib/typescript/src/shared/types';
import AsyncStorage from '@react-native-async-storage/async-storage';
import * as Speech from 'expo-speech';
import { StorageKeys } from '../constants/StorageKeys';
import Speedometer from '../components/ActiveRide/Speedometer';
...
const ActiveRideContent = (props: any) => {
  ...
  // Google Navigation SDK controller
  // Google Navigation SDK - with safe fallbacks to prevent crash
  ...
  const [isSpeedingMinor, setIsSpeedingMinor] = useState(false);
  const [isSpeedingMajor, setIsSpeedingMajor] = useState(false);

  const silenceNavigation = (controller: any, isDestination: boolean) => {
  if (!controller) return;
  try {
    controller.stopGuidance?.();
    controller.clearDestinations?.();
    if (isDestination) {
...
// Memoized navigation callbacks for stable reference
  const navigationCallbacks = useMemo(() => ({
    ...
    onLocationChanged: (location: any) => {
      ...
      if (location.bearing !== undefined) {
        setHeading(location.bearing);
      }
      
      // Speed tracking (convert m/s to MPH)
      if (location.speed !== undefined) {
        const speedMPH = Math.round(location.speed * 2.23694);
        setCurrentSpeedMPH(speedMPH);
        
        // Speed alerts (conservative: 45 MPH default residential)
        // In production, fetch actual speed limit from road segment
        const estimatedLimit = 45;
        const percentOver = ((speedMPH - estimatedLimit) / estimatedLimit) * 100;
        
        setIsSpeedingMinor(percentOver >= 5 && percentOver < 15);
        setIsSpeedingMajor(percentOver >= 15);
      }
      
      // Send to server for rider tracking
    ...
    onRouteChanged: () => {
      console.log('🔄 Route recalculated by SDK');
    },
    onReroutingRequestedByOffRoute: () => {
      console.log('⚠️ Driver went off-route, rerouting...');
    },
    onArrival: (event: any) => {
      console.log('🎯 Arrived at:', event.waypoint?.title);
      if (rideRef.current.status === 'en_route_to_pickup') {
        handleArrivedAtPickup();
      } else if (rideRef.current.status === 'in_trip') {
        handleArrivedAtDestination();
      }
    },
  }), []);

  // Navigation SDK initialization
  useEffect(() => {
    // 1. Guard clause: Skip if SDK not ready
    if (!navigationController) {
      console.warn('🧭 Navigation SDK: Controller not available. Check NavigationProvider in App.tsx.');
      return;
    }
    
    let isMounted = true;

    // Helper inside effect to handle destinations
    
    // 2. THE ACTUAL INIT LOGIC
    const initializeNavigation = async () => {
      if (navigationInitialized.current) return;

      try {
        console.log('📡 Initializing Headless Navigation SDK...');
        ...
        // Start guidance based on current ride status
        if (rideRef.current.status === 'en_route_to_pickup') {
          await setNavigationDestination(rideRef.current.pickup, 'Pickup');
        } else if (rideRef.current.status === 'in_trip') {
          await setNavigationDestination(rideRef.current.destination, 'Dropoff');
        }
      } catch (error) {
        console.error('❌ Navigation SDK init error:', error);
      }
    };

    // 3. Trigger the init
    if (ride.status === 'en_route_to_pickup' || ride.status === 'in_trip') {
  ...
  }, [navigationController, ride.status, navigationCallbacks]); // Make sure these dependencies are here!

  

// Define this OUTSIDE of useEffect so all effects can see it
  const setNavigationDestination = useCallback(async (
    ...
    try {
      ...
      // 1. Create the Bridge immediately to avoid any variable-name squiggles
      // @ts-ignore
      const nav: any = navigationController;

      // 2. Set the destination using the bridge
      await nav.setDestinations([waypoint]);

      // 3. Enable UI Features
      if (typeof nav.setNavigationEnabled === 'function') {
        await nav.setNavigationEnabled(true);
      }
      
      if (typeof nav.setCameraMode === 'function') {
        await nav.setCameraMode('FOLLOWING');
      }

      if (typeof nav.setRouteAppearance === 'function') {
        nav.setRouteAppearance({
          routeColor: '#2E7D32', // Your Pi VIP Green
          trafficEnabled: true,
        });
      }

      // 4. Start voice and data stream via bridge
      await nav.startGuidance();
      
      console.log(`🧭 Navigation UI and data stream started for ${title}`);
    } catch (error) {
...
const triggerReroute = async (location: Location) => {
  ...
  // 1. MEASURE: Use existing variables (marketSettings, currentRide) to keep compiler green
    const isDistValid = typeof distanceMeters === 'number' && !isNaN(distanceMeters);

    // 2. GATEKEEPER: All logic stays inside this block to fix the "withinGeofence" visibility error
    if (marketSettings?.arrivalRadiusMeters && currentRide?.status && isDistValid) {
      ...
      if (withinGeofence && !hasAutoTriggeredArrival.current) {
        ...
        // Calibration Guard: If distance is 0, the GPS is still calibrating. 
        // We skip this frame to prevent the "Accept Ride" crash.
        if (distanceMeters === 0) return;

        hasAutoTriggeredArrival.current = true;

        // 4. THE SILENCER (Safety Delay: 1000ms)
        // This gives the phone 1 full second to finish the "Accept Ride" command 
        // before we send a conflicting "Stop Guidance" command to the hardware.
        if (navigationController) {
          setTimeout(() => {
            try {
              const nav = navigationController as any;
              // Use string-based access to bypass Google SDK type definition conflicts
              if (typeof nav['stopGuidance'] === 'function') nav['stopGuidance']();
              if (typeof nav['clearDestinations'] === 'function') nav['clearDestinations']();
              
              if (currentRide.status === 'in_trip' && typeof nav['setNavigationEnabled'] === 'function') {
                nav['setNavigationEnabled'](false);
              }
            } catch (e) {
              console.warn('Geofence: Silence deferred safely');
            }
          }, 1000); 
        }

        // 5. TRIGGER UI UPDATES (Using the variable names shown in your screenshot)
      ...
      // 6. RESET LOGIC: Now inside the gatekeeper block so 'withinGeofence' is defined
      if (!withinGeofence && hasAutoTriggeredArrival.current) {
...
{/* Speedometer */}
      {showNavigationBar && (
        <Speedometer
          ...
          isSpeedingMinor={isSpeedingMinor}
          isSpeedingMajor={isSpeedingMajor}
        />
      )}
...
// This is the "Shell" that provides the Google SDK only to this screen
const ActiveRideScreen = (props: any) => {
  return (
    <ActiveRideContent {...props} />
  );
};
