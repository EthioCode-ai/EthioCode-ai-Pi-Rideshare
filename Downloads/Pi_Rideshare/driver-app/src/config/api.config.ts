...
type Environment = 'production' | 'production';
type AppRole = 'driver' | 'rider' | 'admin';

// 👇 CHANGE THESE TO SWITCH ENVIRONMENTS/APPS
const CURRENT_ENV: Environment = 'production'; // Switch to 'local' for local testing
const APP_ROLE: AppRole = 'driver'; // Change to 'rider' or 'admin' for other apps

const API_URLS = {
  local: `http://${LOCAL_IP}:${LOCAL_PORT}`,
  production: 'https://ethiocode-ai-pi-rideshare.onrender.com', // ← YOUR RENDER URL
};
// ✅ Export API_BASE_URL based on current environment
