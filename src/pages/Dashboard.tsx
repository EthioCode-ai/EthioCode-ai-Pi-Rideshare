...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    socketConnection.on('driver-location-update', (data) => {
      ...
      setRealDrivers(prevDrivers => {
        ...
        if (data.isAvailable && data.location) {
          updated.push({
            id: data.driverId,
          ...
          console.log('✅ Added driver to map:', data.driverId, data.location);
        }
        return updated;
      });
    });
  ...
  const fetchHeatmapData = async () => {
    try {
      ...
      const response = await fetch('/api/admin/surge/heatmap', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const fetchSurgeData = async () => {
    try {
      ...
      const [zonesRes, timeRulesRes, weatherRulesRes, configRes, statusRes] = await Promise.all([
        fetch('/api/admin/surge/zones', { headers }).catch(() => ({ ok: false, json: () => Promise.resolve([]) })),
        fetch('/api/admin/surge/time-rules', { headers }).catch(() => ({ ok: false, json: () => Promise.resolve([]) })),
        fetch('/api/admin/surge/weather-rules', { headers }).catch(() => ({ ok: false, json: () => Promise.resolve([]) })),
        fetch('/api/admin/surge/algorithm-config', { headers }).catch(() => ({ ok: false, json: () => Promise.resolve({}) })),
        fetch('/api/admin/surge/current-status', { headers }).catch(() => ({ ok: false, json: () => Promise.resolve({}) }))
      ]);
  ...
  const removeSurgeOverride = async (zoneId: string) => {
    try {
      ...
      const response = await fetch(`/api/admin/surge/override/${zoneId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  useEffect(() => {
    const loadGoogleMaps = async () => {
      try {
        ...
        const response = await fetch('/api/config/maps-key').catch(() => null);
        if (!response || !response.ok) {
  ...
  // Helper function to create professional TopDown car SVG for Google Maps
  const createTopDownCarIcon = (color: string, rotation: number = 0, size: number = 40) => {
    const width = size;
      ...
      url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(`
        <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
          <g transform="rotate(${rotation} ${width/2} ${height/2})">
            ...
            <!-- Rear Window -->
            <rect x="${width*0.15}" y="${height*0.55}" 
            ...
            <!-- Side Windows -->
            <rect x="${width*0.1}" y="${height*0.35}" 
                  width="${width*0.8}" height="${height*0.3}" 
                  fill="rgba(135, 206, 235, 0.4)" 
                  rx="2" ry="2"/>
            
            <!-- Headlights -->
            <rect x="${width*0.2}" y="${height*0.05}" 
                  width="${width*0.15}" height="${height*0.08}" 
                  fill="rgba(255, 255, 255, 0.8)" 
                  rx="2" ry="2"/>
            <rect x="${width*0.65}" y="${height*0.05}" 
                  width="${width*0.15}" height="${height*0.08}" 
                  fill="rgba(255, 255, 255, 0.8)" 
                  rx="2" ry="2"/>
            
            <!-- Taillights -->
            <rect x="${width*0.2}" y="${height*0.87}" 
                  width="${width*0.15}" height="${height*0.08}" 
                  fill="rgba(220, 38, 38, 0.8)" 
                  rx="2" ry="2"/>
            <rect x="${width*0.65}" y="${height*0.87}" 
                  width="${width*0.15}" height="${height*0.08}" 
                  fill="rgba(220, 38, 38, 0.8)" 
                  rx="2" ry="2"/>
          </g>
  ...
  const initializeGoogleMaps = () => {
    ...
    if (!apiKey || apiKey === 'YOUR_GOOGLE_MAPS_API_KEY') {
      console.error('❌ Google Maps API key not found. Please check your Replit Secrets.');
