...
const io = socketIo(server, {
  ...
  cors: {
    origin: process.env.NODE_ENV === 'production'
      ? ['https://yourdomain.com', 'capacitor://localhost', 'ionic://localhost', 'http://localhost', 'https://localhost']
      : true,         // Allow all origins in development
    methods: ["GET", "POST"],
    credentials: true
  },
  allowRequest: (req, callback) => {
    // Allow all requests - more permissive for development and mobile apps
    callback(null, true);
  }
});
...
app.use(cors({
  origin: process.env.NODE_ENV === 'production'
    ? ['https://yourdomain.com', 'capacitor://localhost', 'ionic://localhost', 'http://localhost', 'https://localhost'] // Allow mobile apps and actual domain
    : true,
  credentials: true
}));
...
let vehicleTypes = [
  { id: 'economy', name: 'Economy', description: 'Compact cars • Budget-friendly', icon: '/cars/economy.png', baseFare: 2.10, capacity: 4, enabled: true },
  { id: 'standard', name: 'Standard', description: 'Mid-size sedans • Comfortable', icon: '/cars/standard.png', baseFare: 2.50, capacity: 4, enabled: true },
  { id: 'xl', name: 'XL', description: 'SUVs & minivans • Up to 6 passengers', icon: '/cars/xl.png', baseFare: 3.50, capacity: 6, enabled: true },
  { id: 'premium', name: 'Premium', description: 'Luxury vehicles • Premium experience', icon: '/cars/premium.png', baseFare: 4.50, capacity: 4, enabled: true }
];
  ...
  socket.on('driver-connect', (data) => {
    ...
    io.emit('driver-availability-update', {
      ...
      vehicle: {
        ...
        markerImage: '/markers/car-marker.png'
      },
...
// Periodic cleanup to sync database with actual socket connections (every 60 seconds)
setInterval(async () => {
